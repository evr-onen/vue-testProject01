<template>
	<div class="cartItem w-full h-32 grid grid-cols-10 border-l border-r border-b border-rose-200">
		<div class="col-span-2 h-32 p-2 w-full flex justify-center items-center">
			<img class="h-full w-auto object-contain" :src="products[productIndex].images[0]" :alt="product.name" />
		</div>
		<div class="col-span-2 py-2 flex flex-col justify-center items-start">
			<p class="font-cinzel font-bold">{{ products[productIndex].name }}</p>
			<p class="my-0.5 font-condensed text-sm font-semibold" v-for="(valueItem, key) in product.variantData" :key="key">
				{{ key }} :
				{{ valueItem }}
			</p>
		</div>
		<div class="col-span-2 flex">
			<p class="my-auto">{{ finalPrice(products[productIndex], variant) }} ₺</p>
		</div>
		<div class="col-span-2 flex">
			<div class="my-auto">
		...
		<div class="col-span-1 flex">
			<p class="my-auto">{{ dotIndicator(price) }} ₺</p>
		</div>
		<div class="col-span-1 flex justify-end items-center pr-4">
			<span
