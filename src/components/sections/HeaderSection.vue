			...
			<div class="buttons h-full flex">
				<div class="pages h-full">
					<ul class="flex h-full items-center">
						<li class="h-full" v-for="(link, index) in headerPageLinks" :key="index">
				...
				<div class="btns flex h-full ml-4">
					<span class="h-full flex items-center cursor-pointer"><i class="magniferIcon h-6 w-6 block"></i></span>
					<router-link class="" to="/cart">
						<span class="h-full flex items-center ml-4 cursor-pointer relative">
							<span
								class="circle bg-black text-white rounded-full w-4 h-4 absolute top-5 -right-2 text-xs flex items-center justify-center"
								>{{ totalProductsQuantity() }}</span
							>
							<svg
...
const totalProductsQuantity = () => {
	let totalQuantity = 0;
	cartProducts.value.map((cartItem) => (totalQuantity += cartItem.quantity));
